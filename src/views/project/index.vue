<template>
  <n-card title="区域管理" class="area-card">
    ...
    <n-empty v-if="!areas.length && !areaLoading" description="暂无数据">
    </n-empty>
    <n-spin :show="areaLoading">
      <n-grid x-gap="10" :cols="2">
        <n-gi v-for="(item, index) in areas">
          <n-card :title="item.name" size="small" :closable="!Boolean(item.cities.length)" class="area-card"
            @close="() => { handleCardClose(item) }">
            <n-space>
              <n-tag v-for="city in item.cities" closable @close="handleDelCity(index, city)">
                {{ city.label }}
              </n-tag>
              <n-input v-show="item.isEdit" type="text" size="small" autofocus
                @keyup.enter="handleCreateCity(index, $event.target.value)" placeholder="输入城市名，回车键确定">
                <template #suffix>
              ...
              <n-button v-show="!item.isEdit" size="small" dashed @click="() => { item.isEdit = true }">
                <template #icon>
                  <n-icon>
                    <PlusOutlined />
                  </n-icon>
...
const loadDataTable = async (params) => {
  const res = await getResourceList({ ...searchFormRef.value.searchParams, ...params, resourceType: 'PROJECT' });
  return res;
};
